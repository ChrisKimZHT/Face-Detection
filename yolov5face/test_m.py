...
# from AdaFace.face_alignment.mtcnn_pytorch.src import detect_faces
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='/root/PycharmProjects/pythonProject/yolov5-face/yolov5s-face.pt', help='model.pt path(s)')
    ...
    parser.add_argument('--img_path',default ="test.txt",type=str)
    parser.add_argument('--data_root',default="./data",type=str)
    parser.add_argument('--data_box', default="./data_box", type=str)
    parser.add_argument('--data_face', default="/root/data_face", type=str)
    ...
    video_root = '/root/PycharmProjects/pythonProject'
    VideoName = 'video.mov'
    root_name = os.path.join(video_root,VideoName)
    face_root = './data_face'
    video_save_root = './'
    radio = 0.15
    with torch.no_grad():
        cap = cv2.VideoCapture('/root/PycharmProjects/pythonProject/video.mov')
        tracker = Sort()
        frame_num=0
        ...
        while cap.isOpened():
            ...
            for box in boxes:
                ...
                if(float(confidence)>=radio):
                    detections.append([x, y, x+width, y+height, float(confidence)])
            ...
            try:
                ...
                for i, track in enumerate(tracks):
                    ...
                    cv2.rectangle(frame, (int(x), int(y)), (int(x1), int(y1)), (255, 0, 0), 2)
                    cv2.putText(frame, str(int(track_id)), (int(x), int(y - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.75,
                                (0, 0, 255), 5)
            except Exception as e:
                print(e)
                print(f"No people:{frame_num:0>6}")
    ...
    # with open(opt.img_path,'r') as f:
    #     img_folder = [os.path.join(opt.data_root,line).strip() for line in f.readlines()]
    #     f.close()
    # os.makedirs(opt.data_box, exist_ok=True)
    # os.makedirs(opt.data_face,exist_ok=True)
    #
    # count_face = 0
    # # Load model
    # device = select_device(opt.device)
    ...
    #
    #     for image_path in tqdm(img_folder):
    ...
    #             x, y, width, height,gravity= box
    #             if gravity <0.5:
    #                 frame_num += 1
    #                 continue
    #             face = img0[y:y + height, x:x + width]
    #             face = cv2.resize(face, (112, 112))
    #             face_path = os.path.join(opt.data_face,VideoName.split(".")[-2]+f'{frame_num:0>6}'+f'_{i:0>6}_{gravity}.jpg')
    ...
    #             frame_num+=1
            ...
            #     if _ <0.4 :
            #         continue
            #     cv2.rectangle(img0, (x, y), (x + width, y + height), (0, 0, 255), 2)  # 红色框，线宽为2
            # visualized_image_path = image_path.split("/")[-1]
            # cv2.imwrite(os.path.join(opt.data_box,visualized_image_path), img0)
